            ...
            foreach ($employees as $employee) {
                ...
                foreach ($days as $day) {
                    if (in_array($day->format('Y-m-d'), $RequestedWorkDays) && in_array($day->format('Y-m-d'), $CreatedWorkDays)) {
                        ...
                        $fullShifts[]=['work_date'=>$day, 'company_membership_id'=>$companyMembershipId, 'employee_id'=>$employee->id, 'requested'=>['start_time'=>substr($requestedStartTime, 0, 5), 'end_time'=>substr($requestedEndTime, 0, 5)], 'created'=>['start_time'=>substr($createdStartTime, 0, 5), 'end_time'=>substr($createdEndTime, 0, 5)]];
                    } else if (!in_array($day->format('Y-m-d'), $RequestedWorkDays) && in_array($day->format('Y-m-d'), $CreatedWorkDays)) {
                        $createdStartTime = $CreateShiftWithMemberships->where('work_date', $day->format('Y-m-d'))->first()->start_time;
                        $createdEndTime = $CreateShiftWithMemberships->where('work_date', $day->format('Y-m-d'))->first()->end_time;
                        $fullShifts[]=['work_date'=>$day, 'company_membership_id'=>$companyMembershipId, 'employee_id'=>$employee->id, 'requested'=>['start_time'=>'', 'end_time'=>''], 'created'=>['start_time'=>substr($createdStartTime, 0, 5), 'end_time'=>substr($createdEndTime, 0, 5)]];
                    } else if (in_array($day->format('Y-m-d'), $RequestedWorkDays) && !in_array($day->format('Y-m-d'), $CreatedWorkDays)) {
                        $requestedStartTime = $RequestShiftWithMemberships->where('work_date', $day->format('Y-m-d'))->first()->start_time;
                        $requestedEndTime = $RequestShiftWithMemberships->where('work_date', $day->format('Y-m-d'))->first()->end_time;
                        $fullShifts[]=['work_date'=>$day, 'company_membership_id'=>$companyMembershipId, 'employee_id'=>$employee->id, 'requested'=>['start_time'=>substr($requestedStartTime, 0, 5), 'end_time'=>substr($requestedEndTime, 0, 5)], 'created'=>['start_time'=>'', 'end_time'=>'']];
                    } else {
                        $fullShifts[]=['work_date'=>$day, 'company_membership_id'=>$companyMembershipId, 'employee_id'=>$employee->id, 'requested'=>['start_time'=>'', 'end_time'=>''], 'created'=>['start_time'=>'', 'end_time'=>'']];
                    }
                }
            }
            return $fullShifts;

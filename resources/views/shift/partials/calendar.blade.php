...
<table class="w-full text-left whitespace-no-wrap border border-gray-400 text-sm">
    ...
    <tbody id="required-shift-contents">
        @foreach ($fullRequestedShiftsPerEmployee as $fullRequestedShiftPerEmployee)
            <tr>
                @for ($i = 0; $i < 7; $i++)
                    <td class="required-shift-cell border border-gray-400 p-1 sm:p-3 sm:pt-1 day-{{ $fullRequestedShiftPerEmployee[$i]['day']->format('D') }} @if ($fullRequestedShiftPerEmployee[$i]['requested'] && $fullRequestedShiftPerEmployee[$i]['day']->format('m') == $days[0]->format('m')) bg-my-accent-color-lighter cursor-pointer register @elseif($fullRequestedShiftPerEmployee[$i]['day']->format('m') == $days[0]->format('m')) bg-white cursor-pointer  @else bg-gray-200 opacity-70 pointer-events-none @endif">
                        ...
                        @if ($fullRequestedShiftPerEmployee[$i]['requested'] && $fullRequestedShiftPerEmployee[$i]['day']->format('m') == $days[0]->format('m'))
                            <div class="required-shift flex flex-col items-center justify-center text-sm pointer-events-none">
                                <input type="time" name="start_time[]" value="{{ $fullRequestedShiftPerEmployee[$i]['requested']['start_time'] }}" class="calendar-time input-start-time p-0 border-none bg-my-accent-color-lighter text-sm sm:text-base">
                                <p class="h-1 border-l border-my-text-color"></p>
                                <input type="time" name="end_time[]" value="{{ $fullRequestedShiftPerEmployee[$i]['requested']['end_time'] }}" class="calendar-time input-end-time p-0 border-none bg-my-accent-color-lighter text-sm sm:text-base">
                            </div>
                        @else
                            <div class="invisible tmp-shift flex flex-col items-center justify-center text-sm pointer-events-none">
                                <input type="time" name="start_time[]" value="" class="input-start-time calendar-time p-0 border-none text-sm sm:text-base">
                                <p class="h-1 border-l border-my-text-color"></p>
                                <input type="time" name="end_time[]" value="" class="input-end-time calendar-time p-0 border-none text-sm sm:text-base">
                            </div>
                        @endif
                    </td>
                @endfor

<x-admin-layout>
    <form method="POST" action="{{ route('admin.shift.store') }}">
        ...
        <div class="shadow-custom bg-white flex justify-end w-full fixed -bottom-0 -right-0 py-4 pr-6 font-bold text-sm">
            <input type="submit" id="shiftDetermine" onclick="window.removeEventListener('beforeunload', leavePageConfirm);" class="bg-admin-main-color text-white px-4 py-3 rounded" value="下書きを確定する">
        </div>
    </form>

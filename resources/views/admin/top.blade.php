            ...
            <div class="mx-auto sm:px-6 lg:px-6">
                <div class="flex justify-between sm:h-16 h-10 items-center">
                    ...
                    <div class="flex justify-center items-center">
                        @if (Route::has('admin.login'))
                            <a href="{{ route('top') }}" class="text-center py-2 sm:text-sm text-xs text-my-text-color uppercase tracking-widest hover:opacity-70 transition ease-in-out duration-150">スタッフの方はこちら</a>
                            <a href="{{ route('admin.login') }}" class="hidden sm:block ml-3 w-36 text-center py-2 bg-admin-main-color border border-transparent rounded-full font-semibold text-sm text-white uppercase tracking-widest hover:opacity-70 transition ease-in-out duration-150">ログイン</a>

                            @if (Route::has('register'))
                                <a href="{{ route('admin.register') }}" class="hidden sm:block ml-3 w-36 text-center py-2 font-semibold border border-admin-main-color rounded-full text-sm text-admin-main-color uppercase tracking-widest hover:opacity-70 transition ease-in-out duration-150">アカウント登録</a>
                            @endif
                        @endif

                        <!-- Hamburger -->
                        <div class="flex items-center sm:hidden">
                            <button @click="open = ! open" id="navButton" class="ml-2 inline-flex items-center justify-center p-2 text-white bg-admin-main-color">
                ...
                <div :class="{'block': open, 'hidden': ! open}" id="navContent" class="hidden sm:hidden bg-white">
                    <div class="pt-2 pb-3 space-y-1 text-center">
                        @if (Route::has('admin.login'))
                            <a href="{{ route('admin.login') }}" class="sm:hidden inline-block w-32 text-center py-2 bg-admin-main-color border border-transparent rounded-full font-semibold text-xs text-white uppercase tracking-widest hover:opacity-70 transition ease-in-out duration-150">ログイン</a>

                            @if (Route::has('register'))
                                <a href="{{ route('admin.register') }}" class="sm:hidden inline-block w-32 text-center py-2 font-semibold border border-admin-main-color rounded-full text-xs text-admin-main-color uppercase tracking-widest hover:opacity-70 transition ease-in-out duration-150">アカウント登録</a>
                            @endif
                        @endif
                    </div>
                </div>
